    ...
    if (CP%WantScalars .and. has_cl_2D_array .and. ScalCovFile /= '' .and. CTransScal%NumSources>2) then
        ...
        do in=1,CP%InitPower%nn
            do il=lmin,min(10000,CP%Max_l)
                ...
                write(unit,trim(numcat('(1I6,',(3+num_redshiftwindows)**2))//output_format//')') il, real(outarr)
            end do
            do il=10100,CP%Max_l, 100
                outarr=Cl_scalar_array(il,in,1:3+num_redshiftwindows,1:3+num_redshiftwindows)
                outarr(1:2,:)=sqrt(fact)*outarr(1:2,:)
                outarr(:,1:2)=sqrt(fact)*outarr(:,1:2)
                write(unit,trim(numcat('(1E15.5,',(3+num_redshiftwindows)**2))//output_format//')') real(il), real(outarr)
            end do
        end do
        close(unit)
        deallocate(outarr)
...
!write(*,*) 'TimeSteps npoints:', TimeSteps%npoints
    ...
    !          write (*,'(7'//output_format//')') tau, wing(j2), winV(j2), dwing(j2), dwinV(j2),ddwinV(j2),dopac(j2)
